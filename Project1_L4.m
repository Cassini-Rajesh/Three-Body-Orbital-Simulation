...
L4 = load("EM_L4-304P1.mat");

MU1 = L4.MU1;
T = L4.T;
tspan = linspace(0, T, 1000);
perturbation = L4.perturbation;
x0 = L4.x0;
x0 = x0(:);
...
[t, sol] = ode45(@(t,y) L4SS(t, y, MU1), tspan, x0, options);
...
xlabel('x position')
ylabel('y position')
title('Lyapunov Orbit Around L4 (rotating)')
grid on;
...
xlabel('x position')
ylabel('y position')
title('Lyapunov Orbit Around L4 (inertial)')
grid on;
...
options = odeset('RelTol',1e-9, 'AbsTol',1e-12);
[t, sol_p] = ode45(@(t,y) L4SS(t, y, MU1), tspan, x0_perturbed, options);
...
function jacob = LL4(t, delta_x, A)
...
[t, sol_L] = ode45(@(t, delta_x) LL4(t, delta_x, A), tspan, x0_perturbed', options);
